...
async function subscribeWebhookAsync() {
    var fdata = document.forms[0];

    var formpassword = fdata["password"].value;
    var formurl = fdata["url"].value;
    var formboardtag = fdata["tag"].value;
    var data = await grabJson(`${server}/api/webhooks/subscribe?url=${formurl}&password=${formpassword}&boardtag=${formboardtag}`);
    var obj = JSON.parse(JSON.stringify(data));
    if(obj.id != undefined) {
        alert("Webhook Added");
    }
    else {
        alert(`Failed to add webhook: ${data}`);
    }
    console.log(data);
    return data;
}

async function unsubscribeWebhookAsync() {
    var fdata = document.forms[1];

    var formpassword = fdata["password"].value;
    var formid = fdata["id"].value;
    var data = await grabJson(`${server}/api/webhooks/unsubscribe?password=${formpassword}&id=${formid}`);
    if(data == true) {
        alert("Webhook Removed");
    }
    else {
        alert(`Failed to remove webhook: ${data}`);
    }
    console.log(data);
    return data;
}
